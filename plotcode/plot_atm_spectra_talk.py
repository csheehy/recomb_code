...
fig5=figure(5,figsize=(6,10))
clf()

tit=[u'1000 modes, -40\N{DEGREE SIGN} C amplifiers','1000 modes, cryogenic amplifiers',
     '10,000 modes, cryogenic amplifiers']

#for kk,val in enumerate(Nmodes_arr[0]):
for kk in range(1,2):
    val = Nmodes_arr[kk]
    ...
    #errorbar(bc,y,yerr=dIback.value,xerr=df.value/2,fmt='.k')

    gca().yaxis.set_major_locator(MaxNLocator(4));show()
    grid()

    #title(tit[kk],fontsize=12)
    ylabel(r'Intensity (W/m$^2$/Hz/sr)',fontsize=14)
